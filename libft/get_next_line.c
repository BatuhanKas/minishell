...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/20 13:23:18 by melihyil          #+#    #+#             */
/*   Updated: 2023/09/19 17:54:29 by melihyil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **str)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (ft_free_stash(&stash[fd], 0), 0);
	while (ret > 0)
	{
		...
		if (ft_have_nl(stash[fd]))
		{
			...
			if (!line)
				return (ft_free_stash(&stash[fd], 0), 0);
			return (stash[fd] = ft_recreate_stash(stash[fd]), *str = line, 1);
		}
	}
	return (ft_free_stash(&stash[fd], 1), 0);
}
